...
export default function EpsteinIntelTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Header */}
      <div className="text-xs text-slate-400 p-3 px-3.5 bg-yellow-500/[.08] border border-yellow-500/20 rounded-lg leading-relaxed">
      ...
      {e.keyFindings.map((f, i) => (
        <Card key={i} title={f.title} icon="&#x1F4C4;" accent="#f59e0b" full>
          ...
          <div className="text-xs text-slate-400 leading-relaxed">{f.summary}</div>
        </Card>
      ))}
      ...
      <Card title="Community OSINT Resources" icon="&#x1F517;" accent="#10b981" full>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-2">
          {e.osintResources.map((r, i) => (
            <div key={i} className="p-2 px-2.5 rounded-md bg-white/[.02] border border-white/[.03]">
              <div className="text-xs font-medium text-green-500">{r.name}</div>
              <div className="font-mono text-[9px] text-slate-500 mt-0.5">{r.url}</div>
